...
- **Le scan des brouillons reçoit la connexion de l'exercice** (corrigé à l'implémentation, 2026-09-13) : `scanDraftsFromComptaweb` rouvrait sa propre session via `withAutoReLogin` (contexte `default`). Sans ce threading, la boucle lisait deux exercices d'**écritures** mais un seul de **lignes bancaires** — donc aucun brouillon de septembre, c'est-à-dire l'effet terrain visé qui manquait. La config est optionnelle : absente, le comportement historique est inchangé.
- **Le match de contenu des promotions est borné à l'exercice du snapshot** (ajouté à l'implémentation, 2026-09-13), comme l'est déjà la détection des suppressions. La tolérance de date (±3 j) est absolue : sans cette borne, un brouillon du 30/08 et une ligne CW du 02/09 de même montant et même type pouvaient se promouvoir mutuellement, posant un faux `comptaweb_ecriture_id`. La fenêtre concernée, 29/08–03/09, est précisément celle que cet ADR vise.
- **Un tour dont Comptaweb a invalidé la session est repris une fois** : `clearStoredSession` puis nouvelle tentative. Sans cela, un cookie invalidé avant le TTL de 8 h resterait en cache et ferait échouer cet exercice à chaque cycle, avec un simple avertissement.
