...
| 7. Migration BDD + déploiement | 🟡 prêt à déployer | ADR-017 (Turso) + ADR-018 (Vercel) actés. `db.ts` réécrit en async libsql (76 fichiers convertis). `lib/storage.ts` abstrait FS local (dev) / Vercel Blob (prod). Procédure complète dans [`deployment.md`](deployment.md). Reste : provisionner Turso + Vercel + DNS + tester en réel. |
