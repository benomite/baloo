...
import type { ScrapedCarte } from './cartes-scrape.js';
...
// Couleurs officielles SGDF par branche, basées sur le label renvoyé par
// Comptaweb. Permet d'auto-assigner la couleur à la création d'une unité
// pour que l'affichage graphique (badges dans la table, sélecteurs…) soit
// cohérent dès le premier sync référentiels.
...
function syncCartes(
  ...
  for (const c of cartes) {
    ...
    if (existing) {
      // Mise à jour si un champ a bougé (porteur, code, statut).
      if (
