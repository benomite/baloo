...
function main() {
  ...
  if (existsSync(assoPath)) {
    ...
    for (const s of sections) {
      ...
      const id = `note-asso-${s.title.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 60)}`;
      db.prepare(
  ...
  if (existsSync(finPath)) {
    ...
    for (const s of sections) {
      ...
      if (!s.content) continue;
      const slug = s.title.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 60);
