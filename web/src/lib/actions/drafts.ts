...
// --- Scan : crée un draft par ligne bancaire non rapprochée ---
...
export async function scanDraftsFromComptaweb(): Promise<{ crees: number; existants: number; erreur?: string }> {
  try {
    ...
    for (const c of candidates) {
      const existing = findExisting.get(ctx.groupId, c.ligneBancaireId, c.sousLigneIndex, c.sousLigneIndex);
...
export async function syncDraftToComptaweb(
  ecritureId: string,
  opts: { dryRun?: boolean } = {},
): Promise<{ ok: boolean; message: string; dryRun: boolean; ecritureId?: number; missingFields?: string[] }> {
  try {
    const ctx = getCurrentContext();
    const db = getDb();
